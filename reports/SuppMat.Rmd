...
# Need this for pretty colors in tables
# Run this before knitting!

# md2html <- function(inputFile, outputFile) {
#   require(markdown)
#   markdownToHTML(inputFile, outputFile, stylesheet='style.css')
# }
# 
# options(rstudio.markdownToHTML=md2html)
# 
...
## Supplemental Table 2: UISO cell line samples are not not classified as MCC with a random forest classifier trained using cell line microarray expression data. The mean prediction votes of UISO cell line samples and Mkl-1 and WaGa samples (labeled MCC) are given.

*The MCC samples are used for training so bias may exist. See Supplementary Table 3 for unbiased prediction estimates of Mkl-1 and WaGa samples.

```{r TableS2, echo=FALSE, results='asis'}
rf.eset.nouiso.predict.train <- subset(rf.eset.nouiso.predict.train, cancertype == "MCC")

predictions.combined <- rbind(rf.eset.nouiso.predict, rf.eset.nouiso.predict.train)
avgvotes <- ddply(predictions.combined, .variables="cancertype", colwise(mean, is.numeric))

rownames(avgvotes) <- c("MCC*", "UISO")
avgvotes <- avgvotes[, -1]

print(xtable(avgvotes, digits=3, align='rccccccccc'))
```

## Supplemental Table 3: WaGa and Mkl-1 cell line samples (labeled MCC) held out from classifier training (out-of-bag) are classifed as MCC using random forest trained with cell line microarray expression data.

```{r TableS3, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
sky.xtbl <- xtable(sky.data, align=c('r', 'r', 'c', 'c', 'c', 'c'))

print(sky.xtbl, sanitize.text.function=I, include.rownames=FALSE)
```
