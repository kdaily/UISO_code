...
### Table S1: Median correlation values of cell line-tumor comparisons between Merkel cell polyomavirus positive and negative tumor samples.

```{r TableS1, echo=FALSE, results='asis', include=TRUE}
...
### Table S2: Gene Set Enrichment Analysis (GSEA) on `r nrow(merged.data.wide)` KEGG pathways using differentially expressed genes in the cell lines
...
### Table S3: UISO cell line samples are not classified as MCC with a random forest classifier trained using cell line microarray expression data.

The mean prediction votes of UISO cell line samples and Mkl-1 and WaGa samples (labeled MCC) are given.
^* The MCC samples are used for training so bias may exist.
See Table S4 for unbiased prediction estimates of Mkl-1 and WaGa samples.
...
### Table S4: WaGa and Mkl-1 cell line samples (labeled MCC) held out from classifier training (out-of-bag) are classifed as MCC using random forest trained with cell line microarray expression data.

```{r TableS4, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
### Table S5: Antibodies used in immunohistochemical analysis.
...
### Table S6: Short tandem repeat (STR) fingerprinting results for the WaGa, Mkl-1, and UISO cell lines using 16 markers from the AmpFlSTR Identifiler PCR Amplification Kit (Applied Biosystems).
