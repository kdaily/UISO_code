...
The two kits analyze the same 16 markers, while the Promega kit includes to additional markers (Penta D and E).
...
```{r loadtumordiffexpr, echo=FALSE, error=FALSE, warning=FALSE, cache=FALSE, message=FALSE}
...
### Table S7: Differential expression of MCC tumor samples using clinical parameters.
...
fit2.data <- data.frame(number=fit2.nrows$V1, minqval=fit2.minqval$V1)
rownames(fit2.data) <- c("MCV Status", "Primary vs. Metastasis", "Recurrence", "Stage")

colnames(fit2.data) <- c("# Significant Probesets", "Minimum q-value")
fit2.xtbl <- xtable(fit2.data, align=c('r', 'c', 'c'))
