...
samplenames.predict <- c("MC01", "Mkl-1_1", "Mkl-1_2", "Mkl-1_3", "Mkl-1_4", "Mkl-1_5", "Mkl-1_6",
                         ...
                         "UISO_late_1", "UISO_late_2", "UISO_late_3", "UISO_late_4", "UISO_late_5", "UISO_late_6",
                         "UISO_early_1", "UISO_early_2", "UISO_early_3")
...
## esetSel.celllines <- eset.batcheffect[as.character(topgenes.batcheffect.nocelllines$ID), as.character(pdata.celllines$sample)]
## edata.test <- t(exprs(esetSel.celllines))
...
rf.predict <- cbind(as.data.frame(predict(rf.eset, t(exprs(dataset.predict)), type="prob")),
                    data.frame(sample=factor(rep(c(0:4), c(1,6,6,6,3)),
                                             label=c("MC01", "Mkl-1", "WaGa", "UISO_late", "UISO_early"))))
